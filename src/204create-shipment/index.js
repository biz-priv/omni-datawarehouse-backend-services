...
const { getStatus } = require('../shared/dataHelper');
const { snsPublishMessage } = require('../shared/sns');
...
module.exports.handler = async (event) => {
  try {
    ...
    for (let index = 0; index < data.length; index++) {
      try {
        if (get(data[index], 'dynamodb', null) !== 'NewImage') {
          continue;
        }
        ...
        console.info('iviaCSRes', iviaCSRes);
        ...
        if (
          ...
          console.info('houseBills', houseBills);

          // sending update to WorldTrack for all housebill for the shipment id
          for (let index1 = 0; index1 < houseBills.length; index1++) {
            const element = houseBills[index1];

            // WT upadte xml api
            iviaXmlUpdateRes = await iviaSendUpdate(element, iviaCSRes.shipmentId);
            console.info('iviaXmlUpdateRes', JSON.stringify(iviaXmlUpdateRes));
            iviaXmlUpdateResArr = [
              ...iviaXmlUpdateResArr,
              {
        ...
        const resPayload = {
          ...
          errorMsg: iviaCSRes.status === getStatus().FAILED ? JSON.stringify(iviaCSRes.error) : '',
          errorReason: iviaCSRes.status === getStatus().FAILED ? 'IVIA API ERROR' : '',
        };
        console.info('resPayload', resPayload);
        ...
        if (iviaCSRes.status === getStatus().FAILED) {
          const Message = `Reason for failure: \n 
                          ErrorMSG:- ${updatePayload.errorMsg} \n 
                          errorReason:- ${updatePayload.errorReason} \n 
                          ConsolNo:- ${updatePayload.ConsolNo} \n 
                          FK_OrderNo:- ${updatePayload.FK_OrderNo} \n 
                          payloadType:- ${updatePayload.payloadType} \n\n 
                          IVIA Payload:- ${updatePayload.data} \n\n 
                          DB OBJ:- ${JSON.stringify(updatePayload)}
                          `;
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      axios(config)
        ...
        .catch((error) => {
          console.info('error:iviaCreateShipment API', error?.response);
          resolve({
      ...
      console.info('error:iviaCreateShipment', error);
    ...
    try {
      ...
      axios(config)
        .then((response) => {
          ...
          if (
            obj['soap:Envelope']['soap:Body'].WriteTrackingNoteResponse.WriteTrackingNoteResult ===
            'Success'
          ) {
            resolve(obj);
          } else {
      ...
      console.error('error:iviaSendUpdate', error);
      reject(error);
    }
  });
}
