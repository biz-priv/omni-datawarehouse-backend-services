  ...
  try {
    ...
    for (let index = 0; index < sqsEventRecords.length; index++) {
      try {
        ...
        const shipmentAparData = AWS.DynamoDB.Converter.unmarshall(dynamoData.NewImage);
        ...
        if (shipmentAparData.ConsolNo === '0') {
          ...
          parseInt(shipmentAparData.ConsolNo, 10) > 0 &&
          parseInt(shipmentAparData.SeqNo, 10) < 9999
        ) {
