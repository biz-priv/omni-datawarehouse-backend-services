...
module.exports.handler = async (event, context) => {
    try {
        ...
        console.info("🚀 ~ file: shipment_header_table_stream_processor.js:8 ~ exports.handler= ~ event:", JSON.stringify(event));
        ...
        for (const record of get(event, "Records", [])) {
            ...
            if (get(record, "eventName") === "INSERT") {
                ...
                const getIfHouseBillNumberValidRes = await getIfHouseBillNumberValid(orderNo);
                ...
                console.info("🚀 ~ file: shipment_header_table_stream_processor.js:23 ~ event.Records.forEach ~ getItemParam:", getItemParam);
                const item = await dynamoDB.get(getItemParam).promise();
                console.info("🚀 ~ file: shipment_header_table_stream_processor.js:25 ~ event.Records.forEach ~ item:", item);
                if (get(item, "Item", []).length === 0) {
                    return `Item not found.`;
                }
                await processRecord(record, item, get(getIfHouseBillNumberValidRes, "[0]", null));
            }
        }
        return `Successfully processed ${get(event, "Records", []).length} records.`;
    } catch (e) {
        console.error("🚀 ~ file: shipment_header_table_stream_processor.js:117 ~ module.exports.handler= ~ e:", e);
...
async function insertIntoTransactionTable(item) {
    try {
        ...
        console.error("🚀 ~ file: shipment_header_table_stream_processor.js:148 ~ insertIntoTransactionTable ~ err:", err);
        throw err;
    }
}
...
async function sendMessageToQueue(queueMessage) {
    try {
        console.info("🚀 ~ file: shipment_header_table_stream_processor.js:31 ~ event.Records.forEach ~ queueMessage:", queueMessage);
        await sqs.sendMessage(queueMessage).promise();
    } catch (err) {
        console.error("🚀 ~ file: shipment_header_table_stream_processor.js:154 ~ sendMessageToQueue ~ err:", err);
        throw err;
    }
}
